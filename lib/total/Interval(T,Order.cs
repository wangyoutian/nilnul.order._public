...
namespace nilnul.order.total
{
	...
	public partial class Interval<T,TOrder>
		:
		
		IntervalI<T,TOrder>

		where TOrder:OrderI<T>
		...
		//where T:IEquatable<T>
	{
		...
		public nilnul.order.total.StrictFroTotal<T> strictOrder {
			get {
				return nilnul.order.total.StrictFroTotal<T>.Create(order);
			}
		}
		...
		#region ctor
		public Interval(TOrder order, nilnul.order.interval.Cut2<T> left, nilnul.order.interval.Cut2<T> right)
		{
		...
		public Interval(TOrder order, T left,bool eq, T right,bool rightEq)
